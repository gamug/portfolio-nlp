...
| **FR-011** | Every ML-based stage (sentiment, NER, category, `c_summary`) is restructured around a shared Feature/Train/Inference (FTI) class hierarchy: a common base per component (feature extraction, training, inference) that each stage subclasses, rather than four independent sets of module-level functions. A stage with no real training step (category, `c_summary` — both pretrained/zero-shot as shipped) implements a trivial/no-op `Train` component rather than being forced into a fake one. | `src/` exposes one abstract base class per FTI component (feature extraction, training, inference) importable independently of any one stage; sentiment/NER/category/`c_summary` each provide a concrete subclass of all three (category/`c_summary`'s `Train` subclass is a documented no-op, not omitted); `run_pipeline`'s external behavior (idempotency, one-model-at-a-time VRAM budget, `on_progress` callback shape, result-table contents) is bit-for-bit unchanged — every existing hermetic test in `tests/news_nlp/` passes with no behavioral-assertion changes, only import-path/construction changes where a test reaches into stage internals directly. |
| **FR-012** | `sector_summary` (heuristic, deterministic, no model — FR-005) is fully decoupled from the FTI hierarchy in its own dedicated module, including the orchestration entrypoint itself (not just the composition/query helpers already under `news_nlp/sector_summary/` as of 2026-09-14), since it has no Feature/Train/Inference shape to share with the four ML stages. | `run_sector_summary_stage` (or its renamed equivalent) lives under `news_nlp/sector_summary/`, not `pipeline.py`; nothing in that module imports from the FR-011 FTI base classes; `pipeline.run_pipeline` calls into it the same way it calls the four FTI-based stages, so the external orchestration contract (FR-006) is unchanged. |
| **FR-013** | A redesigned `news_nlp.eval` module reuses the FR-011 Inference classes for its own model-scoring step (sampling a real article, running the production (or a candidate) model over it) instead of duplicating model-loading/prediction logic independently, as the current `news_nlp/eval/` package's relationship to `pipeline.py` does today. | The eval module's inference step for a given stage calls the same `Inference` subclass `pipeline.py` uses for that stage (same class, parameterizable model/revision — not a re-implementation with its own `from_pretrained`/forward-pass code); a code-search for a second, independent `AutoModelFor*.from_pretrained` call site duplicating an FTI `Inference` class's own model-loading returns none inside `news_nlp/eval/`. |
...
| **FR-015** | Before spending a judge LLM call on a sampled `(article_id, task, experiment)`, the eval module checks whether a judge verdict already exists for that exact key **and** for the exact same candidate prediction, and reuses it instead of re-judging — real LLM token spend is not repeated on data already tagged under the same experiment, but an `experiment` label that stayed the same while the underlying model's actual output changed (e.g. a retrain into the same fixed local checkpoint path) is never mistaken for "still the same judged thing" (tightened 2026-09-19, TASKS.md T-109 — see §13 item 18). | Running the same `--stage <s> --run-name <experiment>` invocation twice against an unchanged sample population results in zero new judge-LLM calls on the second run (mockable/countable in a hermetic test — the judge client is never invoked for a key already present in the judge table); a genuinely new `article_id`, a different `experiment` value, or an unchanged `experiment` whose candidate prediction for that article changed, always judges fresh — and the verdict recorded for a given run always matches that run's own prediction, never a different run's. |
...
| **FR-018** | The production pipeline's model selection (name, commit SHA, and batch size for each of the four ML stages) is schema-validated (`pipeline_config.PipelineModelsConfig`, `extra="forbid"`) and lives in one git-tracked file, `config/pipeline_models.json`, loaded by `pipeline.py` at import time — not a Python literal constant and not runtime-overridable via any CLI flag or env var. This is a format change only, not a relaxation of constitution AI-behavior #1 ("model selection is pinned, not dynamic"): changing a model still requires editing this file and going through normal PR review, same as editing a `.py` constant did before. `sentiment` has no `batch_size` field (nothing in `sentiment_stage.py` reads one — its batch size is hardcoded to 1). | `pipeline.py` continues to expose `SENTIMENT_MODEL`/`NER_MODEL`/`CATEGORY_MODEL`/`SUMMARY_MODEL`/`MODEL_REVISIONS`/`NER_BATCH_SIZE`/`CATEGORY_BATCH_SIZE`/`SUMMARY_BATCH_SIZE` as real, monkeypatchable module attributes whose values match `config/pipeline_models.json` (regression-tested); an unknown key, a missing stage, or a non-positive `batch_size` in the JSON fails `PipelineModelsConfig` validation with pydantic's own message; a malformed `config/pipeline_models.json` fails on `import pipeline` itself (every entrypoint, `pytest` included) rather than silently degrading; no CLI flag or env var in this repo can point the pipeline at a different config file. |
...
**Pending architecture decision (§13 item 15, FR-011–FR-013, not yet
built)**: the four ML stages currently ship as independent sets of
module-level functions in `pipeline.py`, never designed against a stated
pattern. `PLAN.md` Work item 10 formalizes a shared Feature/Train/Inference
(FTI) class hierarchy across all four — an internal restructuring, not a
change to the fixed-pipeline/single-process decision above, which stays as
stated. `sector_summary` is explicitly excluded from this hierarchy (it has
no Feature/Train/Inference shape — FR-012) and moves fully into its own
module instead.
...
**Planned (§13 item 15, FR-013–FR-016, not yet built)**: `eval_judgement`
splits into two tables — one for the sampled model **inference** being
evaluated, one for the LLM **judge** verdict on it — each carrying
`article_id`, `task`, and `experiment` columns, plus a new confusion-matrix
table (sentiment/category only, keyed by `experiment`). `PLAN.md` Work item
10 has the concrete design; this note exists so a reader of this section
knows the two-table `eval_run`/`eval_judgement` shape above is the
*current*, soon-to-be-superseded contract, not treated as permanent.
...
4. **Model checkpoints are unpinned to a commit SHA** — **resolved
   ...
   out of scope — §14. **Relocated (2026-09-21)**: the name/revision/
   batch-size constants this item pinned now live in the git-tracked
   `config/pipeline_models.json`, schema-validated by
   `src/pipeline_config.py` and loaded by `pipeline.py` at import time —
   FR-018, `PLAN.md` Work item 14. A format change only (Python literals →
   JSON, still PR-reviewed, no runtime override) — constitution
   AI-behavior #1 ("pinned, not dynamic") required no amendment.
...
10. **`c_summary`'s `mean_coverage` is weak (3.02/5) despite the stage's
    ...
    sentiment/NER were. **Re-run post-fix the same day** (`eval_run`
    34): `mean_coverage` improved 3.02→3.64/5 (HT), but
    `pct_with_hallucination` rose 5.4%→8.5% — a previously-invisible
    hallucination gap the cap had been masking, not yet independently
...
15. **The pipeline and evaluation architecture was never a formal spec —
    it grew as a pragmatic solution to a real, immediate necessity.** Four
    ML stages (`pipeline.py`) and the LLM-as-judge harness
    (`news_nlp/eval/`) were each built incrementally, session by session,
    as concrete gaps surfaced (a missing accuracy baseline, a sampling
    bias, a class-imbalance fix) — not designed up front against a stated
    architectural pattern. The result works and has been validated
    end-to-end (Work items 1-9), but it's four independent sets of
    module-level functions rather than a shared, extensible structure, and
    the eval harness has real, concrete friction this session's own
    sentiment-candidate work exposed directly: no way to run two
    experiments' judged data in the same store (`scripts/
    resample_sentiment_v{3,4,5}_2026_09_15.py` each needed their own
    scratch database copy — PLAN.md Work item 9), no persisted confusion
    matrix, no ROC, and no mechanism to avoid re-spending judge-LLM tokens
    on a row already tagged under the same experiment. **New, priority
    #1, pending (2026-09-16)** — formalize the existing shape into a
    Feature/Train/Inference (FTI) class hierarchy shared across all four
...
17. **`tests/` was never covered by this project's own mypy gate.**
    ...
    T-104–T-108.
18. **The judge-verdict reuse mechanism (item 15/FR-015, T-091) could
    silently pair a stale verdict with a changed prediction under an
    unchanged `experiment` label.** `find_verdict_json` keyed reuse on
    `(article_id, task, experiment)` alone; `experiment` doesn't prove
    "same model" — `run_experiment` (item 16/Work item 11) re-resolves
    `candidate_model` to a fixed local checkpoint path
    (`train_sentiment.py`'s `OUTPUT_DIR`/`OUTPUT_DIR_WEIGHTED`), so
    re-running the same `ExperimentSpec` after a retrain reuses that exact
    same label even though the weights — and the prediction — genuinely
    changed. Surfaced directly (2026-09-19) from a user request to check
    whether the reuse mechanism was actually working; confirmed by a
    direct, standalone reproduction before any code changed: two
    `run_eval` calls sharing `candidate_model`/`candidate_revision`, a fake
    model predicting `"positive"` then `"negative"` for the same article —
    run 2's `eval_inference.prediction_json` correctly recorded
    `"negative"`, but its `eval_verdict.verdict_json` still read
    `ideal_label: "positive"` (run 1's verdict, reused with zero judge-LLM
    calls), so run 2's own aggregated metrics were computed against a
    verdict describing a different prediction than the one actually
    produced that run. **Resolved (2026-09-19)** — `find_verdict_json`
    additionally requires the candidate's paired `eval_inference.
    prediction_json` to match; `runner._resolve_verdicts` threads the
    current item's own serialized prediction through. Regression coverage
    at both the unit level (`test_eval_store.py`) and end-to-end
    (`test_eval_runner.py`, directly mirroring the standalone reproduction)
    fails without the fix and passes with it. A separate, smaller,
    disclosed finding from the same investigation: `docs/evaluation.md` had
    one stale 2026-09-15 comment calling `--run-name` "cosmetic only,"
    contradicted by the real T-090 behavior and by two other correct
    passages in the same doc — corrected alongside this, not itself a code
    bug — FR-015 (tightened), `PLAN.md` Work item 13, `TASKS.md`
    T-109–T-110.
