#!/usr/bin/env python
"""One-shot: score a real-article sample with v3 (the neutral-downsampled
sentiment retrain, PLAN.md Work item 9), so the LLM-judge eval harness can
measure its downstream/production-pipeline behavior. v3 was rejected on the
cheaper offline sentence-level/idiom-probe gate before ever reaching a
downstream run (docs/evaluation.md's 2026-09-14 follow-up -- idiom-probe
neutral F1 collapsed to 0.0), so no such script existed until now; added for
symmetry with v4/v5's own resample scripts, in case the downstream number is
ever wanted for completeness despite the offline rejection.

Same mechanics as scripts/resample_sentiment_v4_2026_09_15.py -- version the
current `article_sentiment`, recreate it empty, reprocess a fresh sample
under the candidate model, via monkeypatching `pipeline.SENTIMENT_MODEL` /
`pipeline.MODEL_REVISIONS` in-process for the duration of this script only.
`src/pipeline.py` itself is never edited -- the file on disk, and therefore
the actually-pinned production model, is untouched throughout.

**This is a temporary, reversible experiment, not an adoption.** Run
against a *fresh* scratch copy of the results DB (not v4's or v5's own
scratch copies), so this run can't collide with or be confused with theirs.
The real, shared nlp_.db/nlp.db are never opened for writing.

Usage:
    uv run python scripts/resample_sentiment_v3_2026_09_15.py \\
        --results-db /path/to/scratch_v3.db --source-db /path/to/urls_.db \\
...
floor" section):
    uv run cli/news_nlp_eval.py --stage sentiment --sample-size 2000 --seed 1 \\
        --run-name v3-downsampled --results-db /path/to/scratch_v3.db \\
        --source-db /path/to/urls_.db
"""
...
_V3_CHECKPOINT = str(Path(__file__).resolve().parent.parent / "models" / "finbert-financial-news")
...
def version_sentiment_table(results_db: Path) -> None:
    ...
    try:
        ...
        print(
            ...
            "(restore from this once the v3 eval is done)."
        )
    finally:
        conn.close()
...
def main() -> None:
    ...
    # Point run_sentiment_stage at v3's local checkpoint instead of the
    ...
    pipeline.SENTIMENT_MODEL = _V3_CHECKPOINT
    pipeline.MODEL_REVISIONS[_V3_CHECKPOINT] = "local"
    print(f"Scoring with v3 (neutral-downsampled retrain): {_V3_CHECKPOINT}")
    ...
    try:
        ...
        print(f"Done: {n:,} articles now have v3-scored rows in {_OLD_TABLE!r}.")
        print(
            "Next: uv run cli/news_nlp_eval.py --stage sentiment --sample-size 2000 --seed 1 "
            "--run-name v3-downsampled"
        )
    finally:
        verify.close()
