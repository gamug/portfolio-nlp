#!/usr/bin/env python
"""One-shot: score a real-article sample with v5 (the sec-bert-base base-
checkpoint-swap sentiment retrain, PLAN.md Work item 9), so the LLM-judge
eval harness can measure its downstream/production-pipeline behavior --
the number that actually validated v2 (docs/evaluation.md's 2026-09-13
four-candidate comparison) and v4 (2026-09-15 follow-up).

v5 lost to v2 on every sentence-level and idiom-probe metric (see
docs/evaluation.md's 2026-09-15 base-model-swap follow-up), so this run
was not going to happen automatically per that disposition -- run at the
user's explicit request ("I have a good feeling") despite that.

Same mechanics as scripts/resample_sentiment_v4_2026_09_15.py -- version
the current `article_sentiment`, recreate it empty, reprocess a fresh
sample under the candidate model, via monkeypatching
`pipeline.SENTIMENT_MODEL` / `pipeline.MODEL_REVISIONS` in-process for the
duration of this script only. `src/pipeline.py` itself is never edited --
the file on disk, and therefore the actually-pinned production model, is
untouched throughout.

**This is a temporary, reversible experiment, not an adoption.** Run
against a *fresh* scratch copy of the results DB
(/workspaces/thesis/data/nlp_use_v5.db, copied from nlp_.db before this
script ran) -- not v4's own nlp_use.db, so this run can't collide with or
be confused with v4's already-scored/versioned state there. The real,
shared nlp_.db/nlp.db are never opened for writing.

Usage:
    uv run python scripts/resample_sentiment_v5_2026_09_15.py \\
        --results-db /workspaces/thesis/data/nlp_use_v5.db \\
        --source-db /workspaces/thesis/data/urls_.db \\
...
_V5_CHECKPOINT = str(
    Path(__file__).resolve().parent.parent / "models" / "sec-bert-base-financial-sentiment"
)
...
def version_sentiment_table(results_db: Path) -> None:
    ...
    try:
        ...
        print(
            ...
            "(restore from this once the v5 eval is done)."
        )
    finally:
        conn.close()
...
def main() -> None:
    ...
    # Point run_sentiment_stage at v5's local checkpoint instead of the
    ...
    pipeline.SENTIMENT_MODEL = _V5_CHECKPOINT
    pipeline.MODEL_REVISIONS[_V5_CHECKPOINT] = "local"
    print(f"Scoring with v5 (sec-bert-base base-checkpoint swap): {_V5_CHECKPOINT}")
    ...
    try:
        ...
        print(f"Done: {n:,} articles now have v5-scored rows in {_OLD_TABLE!r}.")
